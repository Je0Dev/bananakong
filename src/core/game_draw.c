...
static void game_draw_title(const Game *g) {
    ...
    if (((int)(g->screen_timer * 2)) % 2 == 0) {
        DrawText("Press ENTER to start", (SCREEN_WIDTH - MeasureText("Press ENTER to start", 24)) / 2, SCREEN_HEIGHT / 2 + 40, 24, WHITE);
    }
}
...
static void game_draw_overlay(const Game *g) {
    ...
    DrawText("Press ENTER to play again", (SCREEN_WIDTH - MeasureText("Press ENTER to play again", 20)) / 2, SCREEN_HEIGHT / 2 + 50, 20, (Color){200, 200, 200, 255});
}
